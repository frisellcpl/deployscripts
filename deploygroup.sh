...
deploy_pms_group() {
    ...
    log_and_echo "--name ${MY_GROUP_NAME} ${PUBLISH_PORT} --desired ${DESIRED_INSTANCES} --min ${MIN_INSTANCES} --max ${MAX_INSTANCES} -m ${MEMORY} -e "CONTAINER_NAME=${CONTAINER_NAME}" -e "RMQ_NODE=${RMQ_NODE}" -e "RMQ_HOST=${RMQ_HOST}" -e "RMQ_PASSWORD=${RMQ_PASSWORD}" ${FULL_IMAGE_NAME}" |grep \\-\\-anti > /dev/null
